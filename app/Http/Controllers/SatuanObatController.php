...
class SatuanObatController extends Controller
{
    ...
    {
        $data['judul'] = 'Satuan Obat';
        return view('master.satuan-obat',$data);
    }

    public function get_data(Request $request)
    {
            $data = DB::table('satuan_obat')->orderBy('id','desc')->get();
    ...
    {
        $id = $request->get('id');
        $data['nama'] = $request->get('nama_satuan');
        ...
        try{
            if($id == ''){
                ...
                DB::table('satuan_obat')->where(array('id' => $id))->update($data);
                $arr = ['status' => '1'];
            }
            DB::commit();
    ...
    public function edit($id){
        $data = DB::table('satuan_obat')->where('id','=',$id)->first();

        return response()->json($data);
    }

    public function hapus($id){
        $data = DB::table('satuan_obat')->delete($id);
        DB::commit();
        return response()->json($data);
